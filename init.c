...
static int svm_spdot(lua_State *L)
{
	...
	THIntTensor *indices = luaT_checkudata(L,2,"torch.IntTensor");
	...
	luaL_argcheck(L,indices->nDimension == 1, 2, "Index tensor is expected to 1D");
	...
	{
		res += sparse_data[i]*dense_data[indices_data[i]];
	}
...
static int svm_spadd(lua_State *L)
{
	...
	THIntTensor *indices = luaT_checkudata(L,3,"torch.IntTensor");
	THFloatTensor *tsparse = luaT_checkudata(L,4,"torch.FloatTensor");

	luaL_argcheck(L,tdense->nDimension == 1, 1, "Dense Matrix is expected to 1D");
	luaL_argcheck(L,indices->nDimension == 1, 3, "Index tensor is expected to 1D");
	...
	for (i=0; i< indices->size[0]; i++)
	{
		dense_data[indices_data[i]] += c*sparse_data[i];
	}
	return 0;
}
